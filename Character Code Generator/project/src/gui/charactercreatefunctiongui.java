...
import codegenerator.functiongenerator;
...
public class charactercreatefunctiongui {
	...
	private File codeTableFile, charArrayCreateFunctionFile;
...
//		JPanel codeFormat = new JPanel();
		...
		codeTableSecondRow.setLayout(new RelativeLayout(RelativeLayout.X_AXIS, 10));
		...
		browseBT.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				String name = codeTableFile.getName();
				String path = codeTableFile.getPath();
				pathTF.setText(path.substring(0, path.length() - name.length()) + "output_" + name);
				
				charArrayCreateFunctionFile = new File(pathTF.getText().toString());
				
				gen = new functiongenerator(codeTableFile, charArrayCreateFunctionFile);
				numberOFCodeInfoLB.setText(Integer.toString(gen.readFileMetaData(codeTableFile)));
			}
		});

		codeTableSecondRow.add(pathLB, new Float(8));
		codeTableSecondRow.add(pathTF, new Float(16));
		codeTableSecondRow.add(browseBT, new Float(4));

		JPanel codeTableThirdRow = new JPanel();
		codeTableThirdRow.setLayout(new GridLayout(1, 2, 50, 10));
