...
public class mainUI extends JFrame {
	...
	public mainUI(String config[]) {
		...
		File newfile = new File(this.getClass().getClassLoader()
				.getResource("").getPath()
				+ "code.asm");
		...
		String date = other.createFixedLengthString(other.LEFT,
				new SimpleDateFormat().format(new Date(System
						.currentTimeMillis())), constants.STRINGLENGTH);
		...
		String introS = String.format(constants.INTROFORM, name, date, space,
				space, space, space);
		String configS = String.format(constants.CONFIGSTRINGFORM, config[0], config[1], config[2], config[3]);
		String out[] = {introS, configS};
		newtab.setText(introS + configS); 
		FileHandler.outputAsm(out, newfile);
	}
